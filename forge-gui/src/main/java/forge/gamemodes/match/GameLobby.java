...
public abstract class GameLobby implements IHasGameType {
    ...
    public boolean hasBattleboxVariant() {
        return data.appliedVariants.contains(GameType.Battlebox)
                || data.appliedVariants.contains(GameType.Battlebox2);
    }
    ...
    public void applyVariant(final GameType variant) {
        ...
        if (!variant.isBattlebox()) {
            data.appliedVariants.remove(GameType.Battlebox);
            data.appliedVariants.remove(GameType.Battlebox2);
        }
        ...
        case Battlebox:
        case Battlebox2:
            // The two Battlebox types are mutually exclusive with each other and with the
            // deck-replacing variants below.
            data.appliedVariants.remove(variant == GameType.Battlebox ? GameType.Battlebox2 : GameType.Battlebox);
    ...
    private boolean validateBattleboxDeck(final Deck deck, final int playerCount, final boolean type2) {
        if (deck == null) {
            SOptionPane.showMessageDialog("Please specify a Battlebox deck for the first player.");
            return false;
        }

        // Type 2 builds the land station from [BasicLandsSet], so [LandStation] is not consulted.
        if (!type2) {
    ...
    public Runnable startGame() {
        ...
        final boolean isBattleboxType2Match = variantTypes.contains(GameType.Battlebox2);
        final boolean isBattleboxMatch = isBattleboxType2Match || variantTypes.contains(GameType.Battlebox);
        ...
        if (isBattleboxMatch && !validateBattleboxDeck(battleboxDeck, activeSlots.size(), isBattleboxType2Match)) {
            return null;
        }
        ...
        for (final LobbySlot slot : activeSlots) {
            ...
            if (isBattleboxMatch) {
                ...
                final CardPool landStation = config.getLandStation(battleboxDeck, activeSlots.size(), isBattleboxType2Match);
                if (landStation != null) {
                    rp.setBattleboxLandStation(landStation.toFlatList());
                }
            }
