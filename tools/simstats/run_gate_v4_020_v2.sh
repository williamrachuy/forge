#!/usr/bin/env bash
# TICKET-V4-020: the V2 gate — Ultron(V2 = V0 fine-tuned on on-policy corpus) vs Default,
...
OUT=$BASE/simstats/out/v4_020_gate_v2
ROUNDS=20
PER_ROUND=50
export ULTRON_NN_EVAL=true
export ULTRON_NN_MODEL_PATH=$BASE/tools/nn/runs/20260727-130315/model.bin
export ULTRON_SIM_MAX_TOP_LEVEL_CANDIDATES=4
export FORGE_SKIP_GROOM=1

mkdir -p "$OUT"
bash "$BASE/tools/simstats/install_watcher.sh" "$OUT" $((ROUNDS * PER_ROUND)) >/dev/null 2>&1 || true
echo "=== V4-020 V2 GATE started $(date) — $ROUNDS rounds x $PER_ROUND games ===" | tee "$OUT/gate.log"
...
for r in $(seq 1 "$ROUNDS"); do
  ...
  sed -e "s/^seed=.*/seed=$seed/" \
      ...
      -e "s/^name=.*/name=v4_020_gate_v2_round_$r/" "$TEMPLATE" > "$cfg"
