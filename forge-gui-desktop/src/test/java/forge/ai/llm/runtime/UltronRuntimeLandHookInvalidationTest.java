...
public class UltronRuntimeLandHookInvalidationTest extends AITest {

    @Test
    public void testLandPlayInvalidatesUltronRuntimeCache() {
        ...
        SpellAbility landPlay = candidateFromHand("Forest", ultron);
        Assert.assertTrue(landPlay.isLandAbility());
        ((PlayerControllerAi) ultron.getController()).playChosenSpellAbility(landPlay);

        UltronRuntimeDecision secondDecision = runtime.choose(
                List.of(filler), UltronStrategicPlan.GameState.MAIN_PHASE);

        Assert.assertTrue(secondDecision.hasChoice(),
                "A land play through the AI controller should refresh cached runtime state");
    ...
    private Game createFourPlayerGame() {
        ...
        Match match = new Match(rules, players, "UltronRuntimeLandHookInvalidationTest");
