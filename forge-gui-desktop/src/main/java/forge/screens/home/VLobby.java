...
public class VLobby implements ILobbyView {
    ...
    public void update(final boolean fullUpdate) {
        ...
        final boolean isBattleboxSelected = lobby.hasBattleboxVariant();
    ...
    void setReady(final int index, final boolean ready) {
        if (ready && decks[index] == null && !lobby.hasAutoGeneratedVariant()
                && !(lobby.hasBattleboxVariant() && index > 0)) {
    ...
    /** @return whether either Battlebox format is applied. */
    boolean hasBattleboxVariant() {
        return lobby.hasBattleboxVariant();
    }
