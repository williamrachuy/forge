...
public class VLobby implements ILobbyView {
    ...
    private final ImmutableList<VariantCheckBox> vntBoxesLocal  =
            ImmutableList.of(vntVanguard, vntMomirBasic, vntMoJhoSto, vntCommander, vntOathbreaker, vntBrawl, vntTinyLeaders, vntPlanechase, vntArchenemy, vntArchenemyRumble, vntBattlebox);
    ...
    void setReady(final int index, final boolean ready) {
        if (ready && decks[index] == null && !lobby.hasAutoGeneratedVariant()
                && !(lobby.hasVariant(GameType.Battlebox) && index > 0)) {
    ...
    private FDeckChooser createDeckChooser(final GameType type, final int iSlot, final boolean ai) {
        ...
        return cachedDeckChoosers.computeIfAbsent(prefKey, (key) -> {
            final GameType gameType = type == GameType.Battlebox ? GameType.Battlebox : forCommander ? type : GameType.Constructed;
